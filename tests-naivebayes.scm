;; Actually cosine-similarity; Bayes got lost along the way
...
(define (term-occurrence-query terms)
  (format #f "
SELECT DISTINCT ?isba, COUNT(DISTINCT ?term), COUNT(DISTINCT ?id), ~A WHERE {
  GRAPH es: {
    ?id stats:isba ?isba .
...
(define (graph-naive-bayes terms)
  (sort
   ...
   (match-sparql-query
    ((_ . isba) (_ . nterms) (_ . total) (_ . counts) ...)    
    (term-occurrence-query terms)    
    (list isba (apply + counts))) ; (map (lambda (c) (/ c nterms)) counts))))
